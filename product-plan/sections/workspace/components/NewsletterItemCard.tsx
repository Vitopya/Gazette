      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <textarea
            ...
            onBlur={(event) => {
              const next = event.currentTarget.value.trim()
              if (next !== item.description) onEditDescription?.(next)
            }}
            rows={2}
            className="mt-1 w-full bg-transparent text-sm text-zinc-600 dark:text-zinc-300 resize-none focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-zinc-900 rounded-sm px-1 -mx-1"
            aria-label="Description de l'item"
          />
            ...
            {item.bullets.map((bullet, index) => (
              <li key={index} className="group/bullet flex items-start gap-2">
                ...
                <input
                  ...
                  onBlur={(event) => {
                    const next = event.currentTarget.value.trim()
                    if (next && next !== bullet) onEditBullet?.(index, next)
                  }}
                  className="flex-1 bg-transparent text-sm text-zinc-700 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-zinc-900 rounded-sm px-1 -mx-1"
